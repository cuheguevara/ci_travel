...
class Trefkategori extends CI_Controller {

    private $nomor_kategori_baru;
    private $table;
    private $primaryKey;
    private $username;
    private $conn;
    ...
    function __construct(){
        ...
        $this->table = "tref_kategori";
        $this->primaryKey = "kategoriID";
        
        /* inisialisasi nomor kategori baru */
        $this->nomor_kategori_baru=  $this->conn->CIT_AUTONUMBER($this->table, $this->primaryKey, 'K', '2');
    }

    public function index(){
        /* inisialisasi variable yang dibutuhkan pada template dan form utama admin */
        $view = array(
            'mainview'=>'view_tref_kategori'
            ,'namaform'=>'FORM kategori'
            ,'information_message'=>'REFERENSI kategori'
            ,'title'=>'DASHBOARD '.APP_NAME);
        
        /* inisialisasi nilai textbox */
        $textbox = array('kategoriID'=>  $this->nomor_kategori_baru
                , 'kategoriNama'=>'');
    ...
    public function delete(){
        ...
        /* mendapatkan variabel post dari text-box id kategori pada view */
        $kategoriID   = $_POST["kategoriID"]; 
        ...
        $result    = $this->db->query($sql
                , array(
                    mysql_escape_string($kategoriID)
                    , mysql_escape_string($kategoriNama)
                    , $_SERVER["REMOTE_ADDR"]
                    , $this->username
                    ),TRUE)->result();
    ...
    public function update(){
        /* mendapatkan variabel post dari text-box nama kategori pada view */
        $kategoriNama = $_POST["kategoriNama"];
        ...
        /* jika nama kategori tidak kosong, maka eksekusi stored procedure penyimpanan kategori */
        if((isset($kategoriNama)) && (trim($kategoriNama)!="")){
            
            /* inisialisasi sql statement untuk memanggil stored procedure */
            $sql       = "call sp_kategori_update(?,?,?,?)";
            ...
            /* jika nama kategori kosong, maka tampilkan pesan error */
    ...
    public function entry(){
        ...
        /* karena proses insert, dan menghindari duplikasi entry pada kolom kategoriID, generate ulang untuk ID kategori */
        $kategoriID   = $this->conn->CIT_AUTONUMBER($this->table, $this->primaryKey, 'K', '2');
    ...
    public function gridkategori(){
        ...
        $data   = $this->conn->CIT_JQGRID('tref_kategori');
        ...
        $row = $this->conn->CIT_JQGRID('tref_kategori',$sidx, $sord, $start, $limit);
        ...
        foreach ($row as $val => $r) {
            $responce->rows[$i]['kategoriID']=$r['kategoriID'];
